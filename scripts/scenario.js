...
function GrammarChecker() {
    ...
    storyWords = storyWords.filter(
        (word) => { return word !== unnecessaryWord;}
    );

    storyWords = storyWords.map(
        (word) => {return word === misspelledWord ? 'beautiful' : word; }
    )

    let badWordIndex = storyWords.findIndex(
        (word) => { return word === badWord; }
    );
    ...
    let lengthCheck = storyWords.every(
        (word => { return word.length < 10; })
    );
    console.log(lengthCheck);

    storyWords.forEach(
        (word) => { word.length > 10 && console.log(word); }
    )
